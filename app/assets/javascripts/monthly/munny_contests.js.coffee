...
LootCrate.monthly['monthly-munnycontest'] = ->
  ...
  loot =
    ...
    bindings: ->
      @navBindings()
      $('.munBtnEnter').on 'click', ->
        $(window).scrollTo $('.' + 'pane5'), 1000, offset: top: -50
        return
      $('.munBtnPrizes').on 'click', ->
    ...
    judgePop: (judge) ->
      ...
      return
    goJbox: (ev) ->
      #loot.vars.lImage = loot.vars.judgeImages[loot.vars.judge];
    ...
    resize: (ev) ->
      #moo($(window).width())
      mvp = $('.munnyContainer').data('mvp')
      jp = $('.munnyContainer').data('jp')
